...
namespace SanguineArchives.BloodyNotify.Systems
{
        ...
        internal static void OnUserOnline(ServerBootstrapSystem sender, NetConnectionId netConnectionId)
        {
            ...
            if (!isNewPlayer)
            {
                if (Database.EnabledFeatures[NotifyFeature.online])
                {
                    ...
                    _message = _message.Replace("#user#", $"{ChatColor.Yellow(userNick)}");
                    ServerChatUtils.SendSystemMessageToAllClients(entityManager, ChatColor.Green($"{_message}"));
